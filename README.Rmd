...
To contribute to this project, some software installations are required, such as `npm`, `node`, and the R package [packer](https://cran.r-project.org/package=packer). Install `node` / `npm` from your platform's usual source and `packer` from CRAN with `install.packages("packer")`.
